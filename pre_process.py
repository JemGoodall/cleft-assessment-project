...
class AudioUtil():
    ...
    # ----------------------------
    @staticmethod
    def spectro_gram(aud, n_mels=64, n_fft=512, hop_len=None):
        sig, sr = aud
        top_db = 80
...
class SoundDS(Dataset):
    def __init__(self, df, data_path, shift=True, mask=True):
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        new_aud = AudioUtil.pad_trunc(rechan, self.duration)
        if self.shift:
            new_aud = AudioUtil.time_shift(new_aud, self.shift_pct)
        sgram = AudioUtil.spectro_gram(new_aud, n_mels=64, n_fft=1024, hop_len=None)
        if self.mask:
