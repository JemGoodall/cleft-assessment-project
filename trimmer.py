'''
Code taken from https://www.kaggle.com/code/christianlillelund/classify-mnist-audio-using-spectrograms-keras-cnn/data
'''
...
def split_multiple_recordings_file(file_path, out_dir, min_silence_duration=0.2, noise_threshold=700):
    """Accepts a file_path of a `wav` file, splits it by it's silent periods and creates new files for each split.
    This is useful when contributing recordings, as it allows one to record multiple pronunciations in one file and then
    ...
    if (len(split_recordings)) == 0:
        print(f"No silence found. Consider increasing noise_threshold (={noise_threshold}) "
    ...
    for idx, recording in enumerate(split_recordings):
        ...
        dir_path = os.path.join(out_dir, new_file_path)
        scipy.io.wavfile.write(dir_path, rate, recording)
...
try:
    ...
    print("Please give input and output directories ", e)

if not os.path.exists(OUTPUT_DIR):
    os.mkdir(OUTPUT_DIR)
    print(f'Making output_dir: {OUTPUT_DIR}')

def trim_from_dir(input_dir, output_dir):
    """Takes an directory of wavs, trims them, and puts them into the output directory

    :param input_dir: directory path of original wavs
    :param output_dir: directory path where trimmed wavs will go
    :return:
    """
    ...
    for subdir, dirs, files in os.walk(INPUT_DIR):
        for filename in files:
            if "wav" in filename:
                ...
                # print(file_path)
                trim_silence_file(file_path)
                split_multiple_recordings_file(file_path, out_dir=output_dir)

trim_from_dir(INPUT_DIR, OUTPUT_DIR)
