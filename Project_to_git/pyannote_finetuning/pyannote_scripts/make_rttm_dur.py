...
def get_valid_list():
    ...
    for file in sorted(os.listdir('../lists')):
        list_path = os.path.join('../lists', file)
...
def make_rttm(input_dir, out_dir, total_files):
    ...
    # get list of valid files (longer than 2 sec)
    valid_file_list = get_valid_list()

    idx = 1
    for filename in sorted(os.listdir(input_dir)):
        file_path = os.path.join(input_dir, filename)
        file_stem = Path(file_path).stem  # get the file name without the extension
        if file_stem in valid_file_list:
            ...
            print(f'{file_stem} too short, skipping')
