'''
For hyper parameter tuning DEPRECIATED!!!!
'''
...
class audioClassifier(nn.Module):
    def __init__(self, kernel_size, max_pool, stride):
        super().__init__()
        ...
        self.pool = nn.MaxPool2d(kernel_size=max_pool, stride=max_pool)  # determine this later! padding=1 because
...
for run_id, (inputD, hiddenD) in enumerate(product(*param_values)):
    ...
    # tb = SummaryWriter(os.path.join(RUN_DIR, name))
    ...
    for data_set_ids in k_list:  # where k_list is [[train_ids, test_ids], [train_ids, test_ids]...]
        ...
        for epoch in range(num_epochs):
            ...
            for i, data in enumerate(train_dl):
                ...
                inputs_m, inputs_s = inputs.mean(), inputs
        ...
        with torch.no_grad():
            ...
            k_testf1 += f1
            k_mae += mae
        print(f'--------- {k_run}/{K} Test results for: {name}, acc={test_acc} f1={f1} mae={mae} ---------')

    print(f'--Final accumulated results for {name}: k_acc={k_testacc} k_f1={k_testf1} k_mae={k_mae}')
